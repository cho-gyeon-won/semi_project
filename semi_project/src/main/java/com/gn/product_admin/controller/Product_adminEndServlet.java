...
public class Product_adminEndServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public Product_adminEndServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			System.out.println(mr.getOriginalFileName("product_img"));
			...
			if(result > 0) {
				view = request.getRequestDispatcher("/product_admin/product_add_success.jsp");
			}
			else {
				view = request.getRequestDispatcher("/product_admin/product_add_fail.jsp");
			}
